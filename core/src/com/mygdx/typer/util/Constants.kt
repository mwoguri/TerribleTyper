package com.mygdx.typer.util

object Constants {

    const val WORLD_SIZE = 2000f
    const val CAMERA_OFFSET_X = WORLD_SIZE / 2
    const val CAMERA_OFFSET_Y = 800

    const val PLAYER_WIDTH = 307

    const val JUMP_SPEED: Float = 200f
    const val GRAVITY: Float = 1200f
    const val FORWARD_VELOCITY: Float = 100f
    const val GROUND_ATLAS = "ground.atlas"
    const val GROUND = "ground"
    const val WALK_ATLAS = "sprite.atlas"
    const val WALK1 = "walk-1"
    const val WALK2 = "walk-2"
    const val WALK3 = "walk-3"
    const val WALK4 = "walk-4"


}